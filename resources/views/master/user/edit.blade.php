<x-dashboard.layout>
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      @endif
      <form action="{{ route('dashboard.master.user.update', ['id' => $user->id]) }}" method="post" class="mb-3">
        @csrf
        @method('put')
        <div class="mb-3">
          <label for="username" class="form-label">Username</label>
          <input type="username" class="form-control @error('username') is-invalid @enderror" name="username" id="username" aria-describedby="username-help" value="{{ $user->username }}">
          @error('username')
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control @error('no_hp') is-invalid @enderror" name="no_hp" id="no_hp" value="{{ $user->no_hp }}">
          @error('no_hp')
            <div class="invalid-feedback">
              {{ $message }}
            </div>
        ...
        <div class="mb-3">
          ...
          <select name="user_type" id="user_type" class="form-control form-select @error('user_type') is-invalid @enderror">
            @foreach ($types as $type)
                <option value="{{ $type['value'] }}" @if ($user->user_type == $type['value'])
                    selected
